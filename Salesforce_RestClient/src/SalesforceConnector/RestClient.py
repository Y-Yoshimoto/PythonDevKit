...
class SfaConnection:
    def __init__(self):
        logger.info("Initialize Salesforce Connecter")
        ## 秘密鍵読み込み
        #f = open('./sfadxinfo.pem', 'r')
        f = open("../certificate/sf_jwt_certificate.key", 'r')
    ...
    def jwt_login(self, consumer_id, username, private_key, sandbox=False):
        ...
        # endpoint = 'https://mydomain.my.salesforce.com' if sandbox is True else 'https://login.salesforce.com'
        ...
        if result.status_code != 200:
            # print(body)
            print(vars(result))
            raise Exception(f"Failed to authenticate with Salesforce: {body.get('error')} - {body.get('error_description')}")
    ...
    def GetLimitsInfo(self):
        ...
        if result.status_code != 200:
            print(vars(result))
            raise Exception(f"Failed to retrieve limits info: {body['error']} - {body['error_description']}")
        return body
