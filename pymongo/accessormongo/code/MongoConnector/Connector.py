...
class Connector:
    """MongoDBコネクター"""
    def __init__ (self, databaseName='Sampledb', endpoint='mongodb://mongo:27017/',user='edituser', password='Password01'):
        ## DBへの接続と認証
        ### 接続先MongoDBの指定
    ...
    def reCreateCollection(self, collectionName: str, datas: list) -> int:
        ...
        self.insertBulkdata(collectionName, datas)
    ...
    def _searchLimitValue(self, collectionName: str, column: str, sort: int):
        ...
        try:
            Max = [data for data in self._db[collectionName].aggregate([match, sort, limit, project])][0]
        except IndexError:
            return "Nodata"
        return Max[column]
