...
### ログ出力 ##############################################################
def nowdate(timezone='+09:00') -> str:
    """ 現在時刻表示文字列 RFC3339 +9H"""
    return datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S.%f"+timezone)

def PrintLogFormat(message: str):
    print(f'{nowdate()} {message} ',flush=True)
